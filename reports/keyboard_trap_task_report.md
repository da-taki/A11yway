...
- Task: Fill the course survey
- Student profile: Keyboard-only student
- Result: BLOCKED at step `focus_favorite_topic` (reason: keyboard_trap)
- Steps passed: 0 of 3

| Step | Action | Status | Announced | Detail |
| --- | --- | --- | --- | --- |
| focus_favorite_topic | focus_by_label_or_name | failed | combobox, "How was the last lesson?", collapsed | Focus loops through button#send_feedback -> button#dismiss_feedback -> select#rating and never reaches this control. |
| type_favorite_topic | type_text | skipped |  | Skipped because an earlier step was blocked. |
| submit_survey | activate_by_role_or_text | skipped |  | Skipped because an earlier step was blocked. |
...
- Task name: Fill the course survey
- Student profile: Keyboard-only student

### Required Actions

- Reach the survey form
- Enter a favorite topic
- Submit the survey

### Likely Blockers

- Keyboard focus is trapped in a loop
  - Issue type: keyboard_trap
  - Severity: high
  - Task impact: This issue may make the task harder to complete.
- Keyboard focus is trapped in a loop
  - Issue type: keyboard_trap
  - Severity: high
  - Task impact: This issue may make the task harder to complete.
...
### 2. Keyboard focus is trapped in a loop
...
- loop_length: 3
- tab_presses: 40
- reason: While searching for this step's control, Tab kept cycling through the same elements without passing through the rest of the page.
